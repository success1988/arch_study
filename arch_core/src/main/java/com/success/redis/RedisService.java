...
public class RedisService {
     ...
     * Logger
     */
    private static final Logger LOGGER = LoggerFactory.getLogger(com.success.redis.RedisService.class);
     ...
     * @param seconds  过期时间
     */
    public void setExpire(final String key, final String value, final long seconds) {
        redisTemplate.execute((RedisCallback<Long>) connection -> {
            ...
            LOGGER.debug("[redisTemplate redis]放入 缓存  url:{} ========缓存时间为{}秒", key, seconds);
            return 1L;
        });
    }
     ...
     * @param time  过期时间
     */
    public void setExpireAt(final String key, final String value, final long time) {
        redisTemplate.execute((RedisCallback<Long>) connection -> {
            RedisSerializer<String> serializer = getRedisSerializer();
            byte[] keys = serializer.serialize(key);
            byte[] values = serializer.serialize(value);
            connection.set(keys, values);
            connection.expireAt(keys, time);
            LOGGER.debug("[redisTemplate redis]放入 缓存  url:{} ========距离缓存到期时间为{}秒", key, time);
            return 1L;
        });
    }
    ...
    public long del(final String... keys) {
        return redisTemplate.execute((RedisCallback<Long>) connection -> {
            long result = 0;
            for (String key : keys) {
            	LOGGER.info("[redisTemplate redis]删除 缓存  url:{} ", key);
    ...
    /**
     * 对某个主键对应的值加value,value值必须是全数字的字符串
     *
     * @param key   the key
     * @param value 增加的值
     * @return the long
    ...
    public long incr(final String key, long value) {
        return redisTemplate.execute((RedisCallback<Long>) connection -> {
            RedisSerializer<String> redisSerializer = getRedisSerializer();
            return connection.incrBy(redisSerializer.serialize(key), value);
        });
    }
